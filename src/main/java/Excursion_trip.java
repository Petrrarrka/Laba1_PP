...
public class Excursion_trip extends Hotel {
    private int price;
    private Ticket ticket = new Ticket();
    private Hotel [] hotels = new Hotel[50];
    private String [] cities = new String[20];
    private int night_amount;

    public void menu()
    {
        System.out.println("Оберіть вид транспорту:\n1 - Літаком\n2 - Автобусом");
        Scanner transport_choice = new Scanner(System.in);
        int transport = transport_choice.nextInt();
        if(transport == 1) {
            print_countries();
            ...
            {
                print_cities();
                System.out.print("Введіть номер міста, або введіть 0 якщо хочете обрати готель - ");
        ...
        {
            print_countries_bus();
            System.out.print("Оберіть країну куди ви хочете поїхати:\n");
            Scanner scanner = new Scanner(System.in);
            String country = scanner.nextLine();
            ticket.setCountry(country);
            System.out.println("Оберіть наступну дію: 1 - обрати Місто 2 - обрати Готель");
            ...
            if(choice == 1)
            {
                ...
                if(check == 0)
                {
                    ...
                    if(hot_ch <= 50 & hot_ch >= 1)
                    {
                        ...
                        setHotelsName(hotels[hot_ch-1].getHotelsName());
                        setPrice(hotels[hot_ch-1].getPrice());
                        setType(hotels[hot_ch-1].getType());
                    }
                }
                if(check >= 1 & check <= 20)
                {
                    fill_hotels_bus();
                    print_local_hotels_bus(cities[check-1]);
                    System.out.println("Введіть номер готелю:");
                    Scanner hotel_choice = new Scanner(System.in);
                    int hot_ch = hotel_choice.nextInt();
                    if(hot_ch <= 50 & hot_ch >= 1)
                ...
                else
                {
                    ...
                    if(hotel_ch <= 50 & hotel_ch >= 1)
                    {
                        ...
                        setHotelsName(hotels[hotel_ch-1].getHotelsName());
                        setPrice(hotels[hotel_ch-1].getPrice());
                        setType(hotels[hotel_ch-1].getType());
                    }
                }
            }
            ...
            System.out.println("Дата виїзду (11.01.2021) - ");
            Scanner scan_date = new Scanner(System.in);
            ticket.setDate(scan_date.nextLine());
            print_ticket_bus();
        }
    ...
    private void print_ticket_bus ()
    {
        System.out.print(" ---------------------- Ваш Білет -----------------------\n ________________________________________________________\n | Дата виїзду - " + ticket.getDate() + "\t | \tКількість людей - " + ticket.getPeople_amount() + "\n | Країна - " + ticket.getCountry()
                + "\t\t | \tМісто - " + getCity() + "\n | Назва житла - " + getHotelsName() + "\t | \t Тип - " + getType());
    ...
    private void print_local_hotels_bus(String city)
    {
        for(int i = 0; hotels[i] != null; i++)
        {
            String str = hotels[i].getCity();
            ...
            if(hotels[i].getCity().equals(city))
            {
                ...
                System.out.println("| Country - " + ticket.getCountry() + " \t City - " + hotels[i].getCity());
                System.out.print("| Type - " + hotels[i].getType());
                System.out.println("\t | \t Price Per night - " + hotels[i].getPrice());
                System.out.println("_______________________________________________\n");
                i++;
            }
    ...
    {
        String fileName = "C://Users//regim//IdeaProjects//Lab_4//Flight//countries//" + ticket.getCountry() + "//hotels//hotels_list.txt";
    ...
    {
        String fileName = "C://Users//regim//IdeaProjects//Lab_4//Flight//countries//" + ticket.getCountry() + "//cities//cities_list.txt";
        try(FileInputStream fin = new FileInputStream(fileName))
        {
            int i = -1,j = 0;
            while((char)(i = fin.read())!= '_'){
    ...
    {
        try(FileInputStream fin = new FileInputStream("C://Users//regim//IdeaProjects//Lab_4//Flight//countries//countries_list.txt")){
            int i=-1;
            while((char)(i = fin.read())!= '_'){
    ...
    private void fill_hotels_bus()
    {
        String fileName = "C://Users//regim//IdeaProjects//Lab_4//Flight//bus_countries//" + ticket.getCountry() + "//hotels//hotels_list.txt";
        try(FileInputStream fin = new FileInputStream(fileName)){
            int i = -1, j = 0;
            while((char)(i = fin.read())!= '_')
            {
    ...
    private void print_cities_bus()
    {
        String fileName = "C://Users//regim//IdeaProjects//Lab_4//Flight//bus_countries//" + ticket.getCountry() + "//cities//cities_list.txt";
        try(FileInputStream fin = new FileInputStream(fileName))
        {
            int i = -1,j = 0;
            while((char)(i = fin.read())!= '_'){
    ...
    private void print_countries_bus()
    {
        try(FileInputStream fin = new FileInputStream("C://Users//regim//IdeaProjects//Lab_4//Flight//bus_countries//bus_countries.txt")){
            int i=-1;
            while((char)(i = fin.read())!= '_'){
                System.out.print(((char)i));
            }
